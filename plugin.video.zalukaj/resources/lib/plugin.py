    ...
    try:
        ...
        for item in zalukaj.fetch_tv_series_seasons_list(link):
            ...
            list_item.setArt({"thumb": item['img'], "poster": item['img'], "banner": item['img'], "icon": item['img'],
                              "landscape": item['img'], "clearlogo": item['img']})
...
def play_tv_series_episode(link_decoded):
    ...
    try:
        link = b64decode(link_decoded)
        movie = zalukaj.fetch_series_single_movie(link)

        if not movie or len(movie) == 0:
        ...
        movie_url = movie[0]['url']

        if len(movie) > 1:
            selected_quality = xbmcgui.Dialog().select("Wybór jakości video", [item['quality'] for item in movie])
            for item in movie:
                if item['quality'] == selected_quality:
                    movie_url = item['url']
                    break
